 ...
 *
 */
class Designate extends HandlerKernel
{
    /**
     * 자주 사용하는 함수 모음
     *
    ...
    public function __invoke(ServerRequestInterface $req, ResponseInterface $res, $args = null)
    {
        ...
        try {
            ...
            if ($params['type'] === 'user') {
                $this->load(CompanyUser::class)->designate($params['company'], $params['target']);
            } else {
                $this->load(CompanyDevice::class)->designate($params['company'], $params['target']);
            }
