...
use App\Device\Domain\User as DeviceUser;
...
class Detail extends HandlerKernel
{
    ...
    public function __invoke(ServerRequestInterface $req, ResponseInterface $res, $args = null)
    {
        ...
        $virtual = $this->load(DeviceVirtual::class)->all($args['device']);

        $handlerLayout = $this->load(HandlerLayout::class);
        $handlerLayout->appendJs('/js/dashboard_device_manager.js');
        return $handlerLayout->render('html/dashboard/device/detail.html', [
            'device'  => $device,
            'virtual' => $virtual,
        ]);
    }
